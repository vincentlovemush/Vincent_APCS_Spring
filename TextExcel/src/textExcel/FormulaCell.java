...
public class FormulaCell extends RealCell{
	...
	public double getDoubleValue (){
		...
		double answer= 0;
		if (splitted[1].charAt(0)<77&&splitted[1].charAt(0)>64&&splitted[1].charAt(1)!='V'){
		...
		for (int i=2; i<splitted.length;i=i+2){
			if(splitted[i].equals("+"))
				if(splitted[i+1].charAt(0)<77&&splitted[i+1].charAt(0)>65&&splitted[1].charAt(1)!='V'){
					SpreadsheetLocation cellLoc= new SpreadsheetLocation (splitted[i+1]);
					answer=answer+((RealCell)(mySpreadsheet.getCell(cellLoc))).getDoubleValue();
				}
				else if (splitted[i+1].equals("AVG")){
				//add everything
				//i++
				}	
				else
